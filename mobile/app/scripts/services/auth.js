...
angular.module('Afrikik')
  	.service('authenticationService',['$http','envConfiguration','TokenHandler','Global','storage','$state','$stateParams', function Authenticator($http, config, TokenHandler, Global, localStorage, $state, $stateParams) {
		...
		var loginWithFBUrl = config[env].host  + config[env].port +'/api/v1/users/session/facebook';
	  ...
	  var authServiceApi ={
		loginWithFacebook:function(user){
			var payload = {
				username: user.email,
				email: user.email,
			...
			return $http.post(loginWithFBUrl, payload).then(function(response){
